    ...
    if num_warns >= limit:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔘 إزالة تحذير", callback_data="rm_warn({})".format(user.id),
                    ),
                ],
            ],
        )
...
__help__ = """
   • `/warns <المستخدم>`*:* عرض عدد وسبب تحذيرات مستخدم.
   • `/warnlist`*:* عرض قائمة جميع فلاتر التحذير الحالية

  *للمشرفين فقط:*
   • `/warn <المستخدم>`*:* تحذير مستخدم. بعد 3 تحذيرات، سيتم حظر المستخدم من القروب. يمكن استخدامه كرد أيضاً.
   • `/dwarn <المستخدم>`*:* تحذير مستخدم وحذف الرسالة. بعد 3 تحذيرات، سيتم حظر المستخدم من القروب. يمكن استخدامه كرد أيضاً.
   • `/resetwarn <المستخدم>`*:* إعادة ضبط تحذيرات مستخدم. يمكن استخدامه كرد أيضاً.
   • `/addwarn <الكلمة> <الرد>`*:* تعيين فلتر تحذير على كلمة معينة. إذا أردت أن تكون الكلمة   جملة، ضعها بين علامتي تنصيص، مثل: `/addwarn "very angry" This is an angry user`.
   • `/nowarn <الكلمة>`*:* إيقاف فلتر تحذير
   • `/warnlimit <رقم>`*:* تعيين حد التحذيرات
   • `/strongwarn <on/yes/off/no>`*:* إذا فُعّل، تجاوز حد التحذيرات سيؤدي إلى حظر. إن لم يُفعّل، سيتم فقط ضرب المستخدم.
  """
