...
__help__ = """
  *للمشرفين فقط:*
   ...
   • `/welcome noformat`*:* عرض إعدادات الترحيب الحالية، بدون تنسيق - مفيد لإعادة استخدام رسائل الترحيب!
   • `/goodbye`*:* نفس استخدام ووسائط `/welcome`.
   • `/setwelcome <نص>`*:* تعيين رسالة ترحيب مخصصة. إذا استُخدم بالرد على وسائط، يستخدم تلك الوسائط.
   • `/setgoodbye <نص>`*:* تعيين رسالة وداع مخصصة. إذا استُخدم بالرد على وسائط، يستخدم تلك الوسائط.
   • `/resetwelcome`*:* إعادة الترحيب للرسالة الافتراضية.
   • `/resetgoodbye`*:* إعادة الوداع للرسالة الافتراضية.
   • `/cleanwelcome <on/off>`*:* عند دخول عضو جديد، حاول حذف رسالة الترحيب السابقة لتجنب إغراق القروب.
   • `/welcomemutehelp`*:* معلومات عن إسكات الترحيب.
   • `/cleanservice <on/off`*:* حذف رسائل خدمة تلغرام للانضمام/المغادرة.
   *مثال:*
  انضم المستخدم للقروب، غادر المستخدم القروب.

  *ماركداون الترحيب:*
   • `/welcomehelp`*:* عرض معلومات تنسيق أكثر لرسائل الترحيب/الوداع المخصصة.
  """
